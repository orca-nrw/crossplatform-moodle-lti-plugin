...
$pageurl = new moodle_url('/mod/orcalti/contentitem_return.php');
...
if (!empty($jwt)) {
    $params = orcalti_convert_from_jwt($id, $jwt);
    ...
    orcalti_verify_oauth_signature($id, $consumerkey);
}
...
if (empty($errormsg) && !empty($items)) {
    try {
        $returndata = orcalti_tool_configuration_from_content_item($id, $messagetype, $version, $consumerkey, $items);
...
$PAGE->requires->js_call_amd('mod_orcalti/contentitem_return', 'init', [$returndata]);
...
if ($errormsg) {
    ...
    // Means success.
    if (!$msg) {
        $msg = get_string('successfullyfetchedtoolconfigurationfromcontent', 'orcalti');
    }
