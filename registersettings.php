...
require_once($CFG->dirroot.'/mod/orcalti/register_form.php');
...
$isupdate = !empty($id);
$pageurl = new moodle_url('/mod/orcalti/registersettings.php');
if ($isupdate) {
    $pageurl->param('id', $id);
}
if (!empty($returnto)) {
...
if ($form->is_cancelled()) {
    ...
    $id = orcalti_add_tool_proxy($data);
    redirect($redirect);
} else {
    ...
    if ($action == 'update') {
        $toolproxy = orcalti_get_tool_proxy_config($id);
        $form->set_data($toolproxy);
        if ($toolproxy->state == ORCALTI_TOOL_PROXY_STATE_ACCEPTED) {
            $form->disable_fields();
        }
    }
    $form->display();
