...
$types = orcalti_get_tools_by_url($starturl, null);
if (!empty($types) && $typeid == -1) {
    // There are matching types for the registration domain, let's prompt the user to upgrade.
    $pageurl = new moodle_url('/mod/orcalti/startorcaltiadvregistration.php');
    $PAGE->set_context($context);
    $PAGE->set_url($pageurl);
    $PAGE->set_pagelayout('maintenance');
    $output = $PAGE->get_renderer('mod_orcalti');
    $page = new \mod_orcalti\output\registration_upgrade_choice_page($types, $starturl);
    echo $output->header();
    ...
    $confurl = new moodle_url('/mod/orcalti/openid-configuration.php');
