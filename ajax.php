...
switch ($action) {
    case 'find_tool_config':
        ...
        require_capability('mod/orcalti:addinstance', $context);

        if (!empty($toolurl) && orcalti_is_cartridge($toolurl)) {
            $response->cartridge = true;
        } else {
            if (empty($toolid) && !empty($toolurl)) {
                $tool = orcalti_get_tool_by_url_match($toolurl, $courseid);
            ...
            if (!empty($toolid)) {
                ...
                if (!$success) {
                    $response->error = s(get_string('tool_config_not_found', 'mod_orcalti'));
                }
            }
        }
