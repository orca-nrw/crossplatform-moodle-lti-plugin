...
/**
 * Helper class for ORCALTI activity.
 *
 * @package    mod_orcalti
...
class helper {
    ...
    /**
     * Get SQL to query DB for ORCALTI tool proxy records.
     *
    ...
    public static function get_tool_proxy_sql(bool $orphanedonly = false, bool $count = false): string {
        if ($count) {
            ...
            // We only want the fields from orcalti_tool_proxies table. Must define every column to be compatible with mysqli.
        ...
        $from = " FROM {orcalti_tool_proxies} ltp";
        if ($orphanedonly) {
