...
/**
 * The mod_orcalti\local\orcaltiservice\service_base class.
 *
 * @package    mod_orcalti
 * @since      Moodle 2.8
 * @copyright  2014 Vital Source Technologies http://vitalsource.com
...
abstract class service_base {
    ...
    public function get_component_id() {
        ...
        return 'orcaltiservice_' . $this->id;
    ...
    /**
     * Default implementation will check for the existence of at least one mod_orcalti entry for that tool and context.
     *
    ...
    public static function get_service_path() {
        ...
        $url = new \moodle_url('/mod/orcalti/services.php');
    ...
    public function check_tool($typeid, $body = null, $scopes = null) {
        ...
        if ($consumerkey === false) {
            ...
            if (!empty($typeid)) {
                $this->type = orcalti_get_type($typeid);
                $this->typeconfig = orcalti_get_type_config($typeid);
                $ok = !empty($this->type->id);
                if ($ok && !empty($this->type->toolproxyid)) {
                    $this->toolproxy = orcalti_get_tool_proxy($this->type->toolproxyid);
                }
            } else {
    ...
    public function check_tool_proxy($toolproxyguid, $body = null) {
        ...
        if (!empty($toolproxyguid)) {
            $toolproxy = orcalti_get_tool_proxy_from_guid($toolproxyguid);
            if ($toolproxy !== false) {
