...
/**
 * A Helper for ORCALTI Dynamic Registration.
 *
 * @package    mod_orcalti
...
/**
 * This class exposes functions for ORCALTI Dynamic Registration.
 *
 * @package    mod_orcalti
 * @copyright  2020 Claude Vervoort (Cengage), Carlos Costa, Adrian Hutchinson (Macgraw Hill)
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class registration_helper {
    ...
    /**
     * Transforms an ORCALTI 1.3 Registration to a Moodle ORCALTI Config.
     *
     ...
     *
     * @return object the Moodle ORCALTI config.
     */
    public function registration_to_config(array $registrationpayload, string $clientid): object {
        ...
        $orcaltitoolconfiguration = $this->get_parameter($registrationpayload,
            'https://purl.imsglobal.org/spec/lti-tool-configuration', true);

        $domain = $this->get_parameter($orcaltitoolconfiguration, 'domain', false);
        $targetlinkuri = $this->get_parameter($orcaltitoolconfiguration, 'target_link_uri', false);
        $customparameters = $this->get_parameter($orcaltitoolconfiguration, 'custom_parameters', false);
        $scopes = explode(" ", $this->get_parameter($registrationpayload, 'scope', false) ?? '');
        $claims = $this->get_parameter($orcaltitoolconfiguration, 'claims', false);
        $messages = $orcaltitoolconfiguration['messages'] ?? [];
        $description = $this->get_parameter($orcaltitoolconfiguration, 'description', false);
        ...
        $domain = orcalti_get_domain_from_url($domain);
        if ($domain !== orcalti_get_domain_from_url($targetlinkuri)) {
        ...
        $config->lti_ltiversion = ORCALTI_VERSION_1P3;
        $config->lti_organizationid_default = ORCALTI_DEFAULT_ORGID_SITEID;
        $config->lti_icon = $logouri;
        $config->lti_coursevisible = ORCALTI_COURSEVISIBLE_PRECONFIGURED;
        ...
        $config->lti_launchcontainer = ORCALTI_LAUNCH_CONTAINER_EMBED_NO_BLOCKS;

        // Sets Service info based on scopes.
        $config->lti_acceptgrades = ORCALTI_SETTING_NEVER;
        $config->orcaltiservice_gradesynchronization = 0;
        $config->orcaltiservice_memberships = 0;
        $config->orcaltiservice_toolsettings = 0;
        if (isset($scopes)) {
            // Sets Assignment and Grade Services info.

            if (in_array(self::SCOPE_SCORE, $scopes)) {
                $config->lti_acceptgrades = ORCALTI_SETTING_DELEGATE;
                $config->orcaltiservice_gradesynchronization = 1;
            }
            ...
            if (in_array(self::SCOPE_NRPS, $scopes)) {
                $config->orcaltiservice_memberships = 1;
            }

            // Sets Tool Settings info.
            if (in_array(self::SCOPE_TOOL_SETTING, $scopes)) {
                $config->orcaltiservice_toolsettings = 1;
            }
        }

        // Sets privacy settings.
        $config->lti_sendname = ORCALTI_SETTING_NEVER;
        $config->lti_sendemailaddr = ORCALTI_SETTING_NEVER;
        if (isset($claims)) {
            // Sets name privacy settings.

            if (in_array('name', $claims)) {
                $config->lti_sendname = ORCALTI_SETTING_ALWAYS;
            }
            ...
            if (in_array('email', $claims)) {
                $config->lti_sendemailaddr = ORCALTI_SETTING_ALWAYS;
            }
        }
        return $config;
    }
    ...
    /**
     * Adds to the config the ORCALTI 1.1 key and sign it with the 1.1 secret.
     *
     * @param array $orcalticonfig reference to orcalticonfig to which to add the 1.1 OAuth info.
     * @param string $key - ORCALTI 1.1 OAuth Key
     * @param string $secret - ORCALTI 1.1 OAuth Secret
     *
     */
    private function add_previous_key_claim(array &$orcalticonfig, string $key, string $secret) {
        if ($key) {
            $oauthconsumer = [];
            ...
            $orcalticonfig['oauth_consumer'] = $oauthconsumer;
        }
    }

    /**
     * Transforms a moodle ORCALTI 1.3 Config to an OAuth/ORCALTI Client Registration.
     *
     * @param object $config Moodle ORCALTI Config.
     * @param int $typeid which is the ORCALTI deployment id.
    ...
    public function config_to_registration(object $config, int $typeid, object $type = null): array {
        ...
        $orcalticonfigurationresponse = [];
        $orcaltiversion = $type ? $type->ltiversion : $config->ltiversion;
        $orcalticonfigurationresponse['version'] = $orcaltiversion;
        if ($orcaltiversion === ORCALTI_VERSION_1P3) {
        ...
        } else if ($orcaltiversion === ORCALTI_VERSION_1 && $type) {
            $this->add_previous_key_claim($orcalticonfigurationresponse, $config->resourcekey, $config->password);
        } else if ($orcaltiversion === ORCALTI_VERSION_2 && $type) {
            $toolproxy = $this->get_tool_proxy($type->toolproxyid);
            $this->add_previous_key_claim($orcalticonfigurationresponse, $toolproxy['guid'], $toolproxy['secret']);
        }
        ...
        $orcalticonfigurationresponse['deployment_id'] = strval($typeid);
        $orcalticonfigurationresponse['target_link_uri'] = $type ? $type->baseurl : $config->toolurl ?? '';
        $orcalticonfigurationresponse['domain'] = $type ? $type->tooldomain : $config->tooldomain ?? '';
        $orcalticonfigurationresponse['description'] = $type ? $type->description ?? '' : $config->description ?? '';
        if ($config->contentitem ?? 0 == 1) {
            ...
            $orcalticonfigurationresponse['messages'] = [$contentitemmessage];
        }
        if (isset($config->customparameters) && !empty($config->customparameters)) {
            ...
            $orcalticonfigurationresponse['custom_parameters'] = $params;
        }
        $scopesresponse = [];
        if ($config->orcaltiservice_gradesynchronization ?? 0 > 0) {
            $scopesresponse[] = self::SCOPE_SCORE;
            $scopesresponse[] = self::SCOPE_RESULT;
            $scopesresponse[] = self::SCOPE_LINEITEM_RO;
        }
        if ($config->orcaltiservice_gradesynchronization ?? 0 == 2) {
            $scopesresponse[] = self::SCOPE_LINEITEM;
        }
        if ($config->orcaltiservice_memberships ?? 0 == 1) {
            $scopesresponse[] = self::SCOPE_NRPS;
        }
        if ($config->orcaltiservice_toolsettings ?? 0 == 1) {
            $scopesresponse[] = self::SCOPE_TOOL_SETTING;
        }
        $registrationresponse['scope'] = implode(' ', $scopesresponse);

        $claimsresponse = ['sub', 'iss'];
        if ($config->sendname ?? '' == ORCALTI_SETTING_ALWAYS) {
            $claimsresponse[] = 'name';
            $claimsresponse[] = 'family_name';
            $claimsresponse[] = 'given_name';
        }
        if ($config->sendemailaddr ?? '' == ORCALTI_SETTING_ALWAYS) {
            $claimsresponse[] = 'email';
        }
        $orcalticonfigurationresponse['claims'] = $claimsresponse;
        $registrationresponse['https://purl.imsglobal.org/spec/lti-tool-configuration'] = $orcalticonfigurationresponse;
    ...
    public function validate_registration_token(string $registrationtokenjwt): array {
        ...
        if ($registrationtoken->scope == self::REG_TOKEN_OP_NEW_REG) {
            ...
            $tool = orcalti_get_type($registrationtoken->sub);
            if (!$tool) {
     ...
     */
    public function orcalti_get_service_scopes() {
    ...
    /**
     * Base64 encoded signature for ORCALTI 1.1 migration.
     * @param string $key ORCALTI 1.1 key
     * @param string $salt Salt value
     * @param string $secret ORCALTI 1.1 secret
     *
    ...
    public function get_tool_proxy(int $proxyid) : array {
        return orcalti_get_tool_proxy($proxyid);
    }
}
