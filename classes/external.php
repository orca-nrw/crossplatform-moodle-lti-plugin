 ...
 * @since      Moodle 3.0
 */
class mod_orcalti_external extends external_api {
    ...
    public static function get_tool_proxies($orphanedonly) {
        ...
        return orcalti_get_tool_proxies($orphanedonly);
    }
    ...
    /**
     * Describes the parameters for get_orcaltis_by_courses.
     *
     * @return external_function_parameters
     * @since Moodle 3.0
     */
    public static function get_orcaltis_by_courses_parameters() {
        return new external_function_parameters (
            array(
    ...
    /**
     * Describes the get_orcaltis_by_courses return value.
     *
     * @return external_single_structure
     * @since Moodle 3.0
     */
    public static function get_orcaltis_by_courses_returns() {
     ...
     * @since Moodle 3.0
     */
    public static function view_orcalti_returns() {
        return new external_single_structure(
            array(
    ...
    public static function create_tool_proxy($name, $registrationurl, $capabilityoffered, $serviceoffered) {
        ...
        $duplicates = orcalti_get_tool_proxies_from_registration_url($registrationurl);
        if (!empty($duplicates)) {
            throw new moodle_exception('duplicateregurl', 'mod_orcalti');
        }
        ...
        $id = orcalti_add_tool_proxy($config);
    ...
    public static function get_tool_proxy_registration_request($id) {
        ...
        $toolproxy = orcalti_get_tool_proxy($id);
        return orcalti_build_registration_request($toolproxy);
    }
    ...
    public static function get_tool_types($toolproxyid) {
        ...
        if (!empty($toolproxyid)) {
            $types = orcalti_get_orcalti_types_from_proxy_id($toolproxyid);
        } else {
    ...
    public static function update_tool_type($id, $name, $description, $state) {
        ...
        if (empty($type)) {
            throw new moodle_exception('unabletofindtooltype', 'mod_orcalti', '', array('id' => $id));
        }
    ...
    public static function delete_tool_type($id) {
        ...
        if (!empty($type)) {
            ...
            $types = orcalti_get_orcalti_types_from_proxy_id($type->toolproxyid);
            if (empty($types)) {
                orcalti_delete_tool_proxy($type->toolproxyid);
            }
        }
    ...
    public static function is_cartridge($url) {
        ...
        $iscartridge = orcalti_is_cartridge($url);
