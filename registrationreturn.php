...
if (empty($top)) {
    ...
    $redirect = new moodle_url('/mod/orcalti/registrationreturn.php', $params);
    $redirect = $redirect->out(false);

    $clickhere = get_string('click_to_continue', 'orcalti', (object)array('link' => $redirect));
    $html = <<< EOD
<html>
<head>
...
EOD;
    ...
    if (!empty($id)) {
        ...
        switch($toolproxy->state) {
            case ORCALTI_TOOL_PROXY_STATE_ACCEPTED:
                $redirect->param('tab', 'tp_accepted');
                break;
            case ORCALTI_TOOL_PROXY_STATE_REJECTED:
                $redirect->param('tab', 'tp_rejected');
                break;
            case ORCALTI_TOOL_PROXY_STATE_PENDING:
