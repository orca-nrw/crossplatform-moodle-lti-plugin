...
$lti = $DB->get_record('orcalti', array('id' => $instanceid));
...
require_capability('mod/orcalti:requesttooladd', context_course::instance($lti->course));

$baseurl = orcalti_get_domain_from_url($lti->toolurl);
...
if (!orcalti_get_tool_by_url_match($lti->toolurl, $lti->course, ORCALTI_TOOL_STATE_ANY)) {
    ...
    $toolconfig->lti_toolurl = orcalti_get_domain_from_url($lti->toolurl);
    $toolconfig->lti_typename = $toolconfig->lti_toolurl;

    orcalti_add_type($tooltype, $toolconfig);

    echo get_string('lti_tool_request_added', 'orcalti');
} else {
    echo get_string('lti_tool_request_existing', 'orcalti');
}
