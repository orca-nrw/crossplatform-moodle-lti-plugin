 ...
 */
function mod_orcalti_verify_private_key() {
    $key = get_config('mod_orcalti', 'privatekey');
    ...
    if (empty($key)) {
        ...
        set_config('kid', $kid, 'mod_orcalti');
        $config = array(
            "digest_alg" => "sha256",
        ...
        if (!empty($privatekey)) {
            set_config('privatekey', $privatekey, 'mod_orcalti');
        } else {
            return get_string('opensslconfiginvalid', 'mod_orcalti');
        }
    }
