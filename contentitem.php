...
$config = orcalti_get_type_type_config($id);
if ($config->lti_ltiversion === ORCALTI_VERSION_1P3) {
    if (!isset($SESSION->lti_initiatelogin_status)) {
        echo orcalti_initiate_login($courseid, 0, null, $config, 'ContentItemSelectionRequest', $title, $text);
        exit;
    } else {
...
// Prepare the request.
$request = orcalti_build_content_item_selection_request($id, $course, $returnurl, $title, $text, [], []);

// Get the launch HTML.
$content = orcalti_post_launch_html($request->params, $request->url, false);
