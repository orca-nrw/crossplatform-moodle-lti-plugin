...
/**
 * xmldb_orcalti_upgrade is the function that upgrades
 ...
 * @return boolean
 */
function xmldb_orcalti_upgrade($oldversion) {
    ...
    if ($oldversion < 2019031300) {
        // Define table orcalti_access_tokens to be updated.
        $table = new xmldb_table('orcalti_types');

        // Define field ltiversion to be added to orcalti_types.
        ...
        if (!$dbman->field_exists($table, $field)) {
            ...
            $DB->set_field_select('orcalti_types', 'ltiversion', 'LTI-1p0', 'toolproxyid IS NULL');
            $DB->set_field_select('orcalti_types', 'ltiversion', 'LTI-2p0', 'toolproxyid IS NOT NULL');
        }

        // Define field clientid to be added to orcalti_types.
        ...
        // Define index clientid (unique) to be added to orcalti_types.
        ...
        upgrade_mod_savepoint(true, 2019031300, 'orcalti');
    }

    if ($oldversion < 2019031301) {
        // Define table orcalti_access_tokens to be created.
        $table = new xmldb_table('orcalti_access_tokens');

        // Adding fields to table orcalti_access_tokens.
        ...
        // Adding keys to table orcalti_access_tokens.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
        $table->add_key('typeid', XMLDB_KEY_FOREIGN, array('typeid'), 'orcalti_types', array('id'));

        // Add an index.
        $table->add_index('token', XMLDB_INDEX_UNIQUE, array('token'));

        // Conditionally launch create table for orcalti_access_tokens.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // Lti savepoint reached.
        upgrade_mod_savepoint(true, 2019031301, 'orcalti');
    }

    if ($oldversion < 2019031302) {
        // Define field typeid to be added to orcalti_tool_settings.
        $table = new xmldb_table('orcalti_tool_settings');
        ...
        // Define key typeid (foreign) to be added to orcalti_tool_settings.
        $table = new xmldb_table('orcalti_tool_settings');
        $key = new xmldb_key('typeid', XMLDB_KEY_FOREIGN, ['typeid'], 'orcalti_types', ['id']);

        // Launch add key typeid.
        $dbman->add_key($table, $key);

        // Lti savepoint reached.
        upgrade_mod_savepoint(true, 2019031302, 'orcalti');
    }
    ...
    if ($oldversion < 2020061501) {
        ...
        // Lti savepoint reached.
        upgrade_mod_savepoint(true, 2020061501, 'orcalti');
    }
