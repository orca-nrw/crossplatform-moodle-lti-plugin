 ...
 * by the restore_orcalti_activity_task
 *
 * @package mod_orcalti
...
class restore_orcalti_activity_structure_step extends restore_activity_structure_step {
    ...
    protected function process_orcalti($data) {
        global $DB;
        ...
        $newitemid = $DB->insert_record('orcalti', $data);
     ...
     * @return void
     */
    protected function process_orcaltitype($data) {
        global $DB, $USER;
     ...
     * @param mixed $data The data from backup XML file
     */
    protected function process_orcaltitoolproxy($data) {
    ...
    protected function process_orcaltitoolsetting($data) {
        ...
        $DB->insert_record('orcalti_tool_settings', $data);
    }
     ...
     * @param mixed $data The data from backup XML file
     */
    protected function process_orcaltisubmission($data) {
        global $DB;

        $data = (object)$data;
        $oldid = $data->id;

        $data->orcaltiid = $this->get_new_parentid('orcalti');
