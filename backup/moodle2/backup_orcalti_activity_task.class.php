...
/**
 * Defines backup_orcalti_activity_task class
 *
 * @package     mod_orcalti
 * @category    backup
...
require_once($CFG->dirroot . '/mod/orcalti/backup/moodle2/backup_orcalti_stepslib.php');

/**
 * Provides the steps to perform one complete backup of the ORCALTI instance
 */
class backup_orcalti_activity_task extends backup_activity_task {
    ...
    /**
     * Defines a backup step to store the instance data in the orcalti.xml file
     */
    protected function define_my_steps() {
        $this->add_step(new backup_orcalti_activity_structure_step('orcalti_structure', 'orcalti.xml'));
    }
    ...
    static public function encode_content_links($content) {
        ...
        // Link to the list of basicorcalti tools.
        $search = "/(".$base."\/mod\/orcalti\/index.php\?id\=)([0-9]+)/";
        $content = preg_replace($search, '$@ORCALTIINDEX*$2@$', $content);

        // Link to basicorcalti view by moduleid.
        $search = "/(".$base."\/mod\/orcalti\/view.php\?id\=)([0-9]+)/";
        $content = preg_replace($search, '$@ORCALTIVIEWBYID*$2@$', $content);
