...
use orcaltisrv_gradebookservices\local\resources\lineitem;
use orcaltisrv_gradebookservices\local\service\gradebookservices;

/**
 * Unit tests for orcalti lineitem.
 *
 * @package    orcaltisrv_gradebookservices
 * @category   test
 * @copyright  2022 Cengage Group <claude.vervoort@cengage.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @coversDefaultClass \mod_orcalti\service\gradebookservices\local\resources\lineitem
 */
class lineitem_test extends \advanced_testcase {
    ...
    public function test_execute_put_nosubreview() {
        ...
        $gbservice->set_type(orcalti_get_type($typeid));
        $this->create_graded_orcalti($typeid, $course, $resourceid, $tag);
    ...
    public function test_execute_put_withsubreview() {
        ...
        $gbservice = new gradebookservices();
        $gbservice->set_type(orcalti_get_type($typeid));
        $this->create_graded_orcalti($typeid, $course, $resourceid, $tag, $subreviewurl, $subreviewparams);
        $gradeitems = $gbservice->get_lineitems($course->id, null, null, null, null, null, $typeid);
        $this->assertEquals(1, $gradeitems[0]);
        $lineitem = gradebookservices::item_for_json($gradeitems[1][0], '', $typeid);
    ...
    private function set_server_for_put(object $course, int $typeid, object $lineitem) {
        $_SERVER['REQUEST_METHOD'] = \mod_orcalti\local\orcaltiservice\resource_base::HTTP_PUT;
        $_SERVER['PATH_INFO'] = "/$course->id/lineitems$lineitem->id";

        $token = orcalti_new_access_token($typeid, ['https://purl.imsglobal.org/spec/lti-ags/scope/lineitem']);
