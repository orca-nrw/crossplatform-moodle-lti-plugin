...
namespace orcaltiservice_gradebookservices\local\resources;
...
class lineitem extends resource_base {
    ...
    private function process_put_request($body, $olditem, $typeid) {
        ...
        $gbs = gradebookservices::find_orcaltiservice_gradebookservice_for_lineitem($olditem->id);
    ...
    private function process_delete_request($item) {
        ...
        if (!$gradeitem->delete('mod/orcaltisrv_gradebookservices')) {
            ...
            $sqlparams = array();
            $sqlparams['id'] = $gbs->id;
            if (!$DB->delete_records('orcaltisrv_gradebookservices', $sqlparams)) {
                throw new \Exception(null, 500);
            }
        }
    }
    ...
    public function parse_value($value) {
        ...
        if (strpos($value, '$LineItem.url') !== false) {
            ...
            if (!empty($id)) {
                ...
                $id = $cm->instance;
                $item = grade_get_grades($COURSE->id, 'mod', 'orcalti', $id);
                if ($item && $item->items) {
