...
namespace orcaltiservice_gradebookservices\local\service;

use orcaltiservice_gradebookservices\local\resources\lineitem;
use orcaltiservice_gradebookservices\local\resources\lineitems;
use orcaltiservice_gradebookservices\local\resources\results;
...
class gradebookservices extends service_base {
    ...
    public function get_permitted_scopes() {
        ...
        if ($ok && isset($this->get_typeconfig()['orcaltiservice_gradesynchronization'])) {
            if (!empty($setting = $this->get_typeconfig()['orcaltiservice_gradesynchronization'])) {
    ...
    public function get_configuration_options(&$mform) {
        ...
        $selectelementname = 'orcaltiservice_gradesynchronization';
    ...
    public function get_launch_parameters($messagetype, $courseid, $user, $typeid, $modlti = null) {
        ...
        $this->set_typeconfig(orcalti_get_type_config($typeid));
        // Only inject parameters if the service is enabled for this tool.
        if (isset($this->get_typeconfig()['orcaltiservice_gradesynchronization'])) {
            if ($this->get_typeconfig()['orcaltiservice_gradesynchronization'] == self::GRADEBOOKSERVICES_READ ||
                    $this->get_typeconfig()['orcaltiservice_gradesynchronization'] == self::GRADEBOOKSERVICES_FULL) {
     ...
     * @param string $typeid
     *
     * @return \orcaltisrv_gradebookservices\local\resources\lineitem|bool
     */
    ...
    public function add_standalone_lineitem(string $courseid, string $label, float $maximumscore,
        ...
        $id = $item->insert('mod/orcaltisrv_gradebookservices');
    ...
    public static function item_for_json($item, $endpoint, $typeid) {
        ...
        $gbs = self::find_orcaltiservice_gradebookservice_for_lineitem($item->id);
        if ($gbs) {
    ...
    public static function find_orcaltiservice_gradebookservice_for_lti($instanceid) {
        ...
        if ($instanceid) {
            $gradeitem = $DB->get_record('grade_items', array('itemmodule' => 'orcalti', 'iteminstance' => $instanceid));
            if ($gradeitem) {
                return self::find_orcaltiservice_gradebookservice_for_lineitem($gradeitem->id);
            }
        }
    }

    /**
     * Find the right element in the orcaltiservice_gradebookservice table for a lineitem
     *
     * @param string $lineitemid The lineitem (gradeitem) id
     * @return object gradebookservice if it exists
     */
    public static function find_orcaltiservice_gradebookservice_for_lineitem($lineitemid) {
        global $DB;
        if ($lineitemid) {
            return $DB->get_record('orcaltisrv_gradebookservices',
