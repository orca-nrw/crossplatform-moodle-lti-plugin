...
/**
 * xmldb_orcaltisrv_gradebookservices_upgrade is the function that upgrades
 * the gradebook orcalti service subplugin database when is needed.
 *
...
function xmldb_orcaltisrv_gradebookservices_upgrade($oldversion) {
    ...
    if ($oldversion < 2020042401) {
        // Define field typeid to be added to orcalti_tool_settings.
        $table = new xmldb_table('orcaltisrv_gradebookservices');
        ...
        upgrade_plugin_savepoint(true, 2020042401, 'orcaltiservice', 'gradebookservices');
    }

    if ($oldversion < 2020042402) {
        // Now that we have added the new column let's migrate it'
        // Prior implementation was storing the resourceid under the grade item idnumber, so moving it to orcalti_gradebookservices.
        // We only care for mod/orcalti grade items as manual columns would already have a matching gradebookservices record.

        $DB->execute("INSERT INTO {orcaltisrv_gradebookservices}
                (gradeitemid, courseid, typeid, orcaltilinkid, resourceid, baseurl, toolproxyid)
         SELECT gi.id, courseid, orcalti.typeid, orcalti.id, gi.idnumber, t.baseurl, t.toolproxyid
           FROM {grade_items} gi
           JOIN {orcalti} orcalti ON orcalti.id=gi.iteminstance AND gi.itemtype='mod' AND gi.itemmodule='orcalti'
           JOIN {lti_types} t ON t.id = orcalti.typeid
          WHERE gi.id NOT IN ( SELECT gradeitemid
                                 FROM {orcaltisrv_gradebookservices} )
             AND gi.idnumber IS NOT NULL
             AND gi.idnumber <> ''");

        // Lti savepoint reached.
        upgrade_plugin_savepoint(true, 2020042402, 'orcaltiservice', 'gradebookservices');
    }

    if ($oldversion < 2020042403) {
        // Here updating the resourceid of pre-existing orcalti_gradebookservices.
        $DB->execute("UPDATE {orcaltisrv_gradebookservices}
                         SET resourceid = (SELECT idnumber FROM {grade_items} WHERE id=gradeitemid)
                       WHERE gradeitemid in (SELECT id FROM {grade_items}
                                             WHERE ((itemtype='mod' AND itemmodule='orcalti') OR itemtype='manual')
                                               AND idnumber IS NOT NULL
                                               AND idnumber <> '')
                         AND (resourceid is null OR resourceid = '')");

        // Lti savepoint reached.
        upgrade_plugin_savepoint(true, 2020042403, 'orcaltiservice', 'gradebookservices');
    }
    ...
    if ($oldversion < 2022051900) {
        ...
        // Lti savepoint reached.
        upgrade_plugin_savepoint(true, 2022051900, 'orcaltiservice', 'gradebookservices');
    }
