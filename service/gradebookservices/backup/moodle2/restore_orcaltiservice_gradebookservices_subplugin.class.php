...
class restore_orcaltisrv_gradebookservices_subplugin extends restore_subplugin {
     ...
     */
    protected function define_orcalti_subplugin_structure() {
    ...
    public function process_orcaltisrv_gradebookservices_lineitem($data) {
        ...
        if ($this->get_mappingid('gbsgradeitemrestored',  $data->id, 0) == 0) {
            $newgbsid = $DB->insert_record('orcaltisrv_gradebookservices', (object) array(
                    'gradeitemid' => 0,
                    'courseid' => $courseid,
    ...
    private function find_proxy_id($data) {
        ...
        $dbtoolproxy = $DB->get_field('orcalti_tool_proxies', 'id', $dbtoolproxyjsonparams, IGNORE_MISSING);
        if ($dbtoolproxy) {
    ...
    /**
     * We call the after_restore_orcalti to update the grade_items id's that we didn't know in the moment of creating
     * the gradebookservices rows.
     */
    protected function after_restore_orcalti() {
        global $DB;
        $activitytask = $this->task;
        $courseid = $activitytask->get_courseid();
        $gbstoupdate = $DB->get_records('orcaltisrv_gradebookservices', array('gradeitemid' => 0, 'courseid' => $courseid));
        ...
        $gi = $DB->get_record('grade_items', array('itemtype' => 'mod', 'itemmodule' => 'orcalti', 'courseid' => $courseid,
