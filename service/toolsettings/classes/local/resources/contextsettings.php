...
namespace orcaltiservice_toolsettings\local\resources;

use orcaltiservice_toolsettings\local\service\toolsettings;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class contextsettings extends \mod_orcalti\local\orcaltiservice\resource_base {

    /**
     * Class constructor.
     *
     * @param \mod_orcalti\local\orcaltiservice\service_base $service Service instance
     */
    ...
    public function execute($response) {
        ...
        if (!$ok) {
            ...
            if (!$ok) {
                ...
                $systemsetting = null;
                $contextsettings = orcalti_get_tool_settings($id, $contextid);
                if (!empty($bubble)) {
                    ...
                    $systemsettings = orcalti_get_tool_settings($id);
                    if ($bubble == 'distinct') {
                ...
                if ($response->get_request_method() == 'GET') {
                    ...
                    if ($ok) {
                        orcalti_set_tool_settings($settings, $id, $contextid);
                    } else {
