 ...
 *
 * @package    orcaltiservice_profile
...
namespace orcaltiservice_profile\local\resources;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class profile extends \mod_orcalti\local\orcaltiservice\resource_base {
    ...
    public function execute($response) {
        ...
        $version = service_base::ORCALTI_VERSION2P0;
        $params = $this->parse_template();
        if (optional_param('lti_version', service_base::ORCALTI_VERSION2P0, PARAM_ALPHANUMEXT) != $version) {
            $ok = false;
            $response->set_code(400);
        } else {
            $toolproxy = orcalti_get_tool_proxy_from_guid($params['tool_proxy_id']);
            $ok = $toolproxy !== false;
        }
        if ($ok) {
            ...
            $serviceoffered = '';
            $sep = '';
            $services = \core_component::get_plugin_list('orcaltiservice');
...
EOD;
            ...
            if (!empty($CFG->mod_orcalti_institution_name)) {
                $consumername = $CFG->mod_orcalti_institution_name;
                $consumerdesc = '';
            } else {
