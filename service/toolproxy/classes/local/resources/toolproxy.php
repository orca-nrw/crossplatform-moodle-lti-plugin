 ...
 *
 * @package    orcaltiservice_toolproxy
...
namespace orcaltiservice_toolproxy\local\resources;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class toolproxy extends \mod_orcalti\local\orcaltiservice\resource_base {

    /**
     * Class constructor.
     *
     * @param orcaltiservice_toolproxy\local\resources\toolproxy $service Service instance
     */
    ...
    public function execute($response) {
        ...
        if ($ok && isset($toolproxyjson->security_contract->tool_service)) {
            $contexts = orcalti_get_contexts($toolproxyjson);
            ...
            foreach ($tpservices as $service) {
                ...
                if (substr($fqid, 0, strlen($context)) !== $context) {
                    ...
                    $id = explode('#', $fqid, 2);
                    $aservice = orcalti_get_service_by_resource_id($services, $id[1]);
        ...
        if ($ok) {
            ...
            foreach ($tools as $tool) {
                ...
                if (isset($messages['ContentItemSelectionRequest'])) {
                    ...
                    $contentitemparams = self::orcalti_extract_parameters($contentitemrequest->parameter);
                    $config->lti_parameter_ContentItemSelectionRequest = $contentitemparams;
                }

                $type = new \stdClass();
                $type->state = ORCALTI_TOOL_STATE_PENDING;
                $type->ltiversion = ORCALTI_VERSION_2;
                ...
                $type->parameter = self::orcalti_extract_parameters($launchrequest->parameter);
                ...
                $ok = $ok && (orcalti_add_type($type, $config) !== false);
            }
            if (isset($toolproxyjson->custom)) {
                orcalti_set_tool_settings($toolproxyjson->custom, $toolproxy->id);
            }
        }
        ...
        if (!empty($toolproxy)) {
            if ($ok) {
                ...
                $toolproxy->state = ORCALTI_TOOL_PROXY_STATE_ACCEPTED;
...
EOD;
                ...
                $toolproxy->state = ORCALTI_TOOL_PROXY_STATE_REJECTED;
     ...
     */
    private static function orcalti_extract_parameters($parameters) {
