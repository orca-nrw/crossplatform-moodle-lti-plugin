...
if ($ok) {
    ...
    $typeconfig = orcalti_get_type_config($tool->id);
    $permittedscopes = orcalti_get_permitted_service_scopes($tool, $typeconfig);
...
if ($ok) {
    $token = orcalti_new_access_token($tool->id, $scopes);
    $expiry = ORCALTI_ACCESS_TOKEN_LIFE;
