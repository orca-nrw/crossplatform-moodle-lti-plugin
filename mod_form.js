...
(function(){
    ...
    M.mod_orcalti = M.mod_orcalti || {};

    M.mod_orcalti.ORCALTI_SETTING_NEVER = 0;
    M.mod_orcalti.ORCALTI_SETTING_ALWAYS = 1;
    M.mod_orcalti.ORCALTI_SETTING_DELEGATE = 2;

    M.mod_orcalti.editor = {
        ...
        updateAutomaticToolMatch: function(field){
            ...
            if(selectedToolType > 0){
                ...
                if(match && match[1] && match[1].toLowerCase() === selectedOption.getAttribute('domain').toLowerCase()){
                    automatchToolDisplay.set('innerHTML',  '<img style="vertical-align:text-bottom" src="' + self.settings.green_check_icon_url + '" />' + M.util.get_string('using_tool_configuration', 'orcalti') + selectedOption.get('text'));
                } else {
                    // The entered URL does not match the domain of the tool configuration
                    automatchToolDisplay.set('innerHTML', '<img style="vertical-align:text-bottom" src="' + self.settings.warning_icon_url + '" />' + M.util.get_string('domain_mismatch', 'orcalti'));
                }
            }
            ...
            if(key.get('value') !== '' && secret.get('value') !== ''){
                automatchToolDisplay.set('innerHTML',  '<img style="vertical-align:text-bottom" src="' + self.settings.green_check_icon_url + '" />' + M.util.get_string('custom_config', 'orcalti'));
            }
            ...
            var continuation = function(toolInfo, inputfield){
                ...
                if(toolInfo.toolname){
                    automatchToolDisplay.set('innerHTML',  '<img style="vertical-align:text-bottom" src="' + self.settings.green_check_icon_url + '" />' + M.util.get_string('using_tool_configuration', 'orcalti') + toolInfo.toolname);
                } else if(!selectedToolType) {
                    // Inform them custom configuration is in use
                    if(key.get('value') === '' || secret.get('value') === ''){
                        automatchToolDisplay.set('innerHTML', '<img style="vertical-align:text-bottom" src="' + self.settings.warning_icon_url + '" />' + M.util.get_string('tool_config_not_found', 'orcalti'));
                    }
                }
                if (toolInfo.cartridge) {
                    automatchToolDisplay.set('innerHTML', '<img style="vertical-align:text-bottom" src="' + self.settings.green_check_icon_url +
                                             '" />' + M.util.get_string('using_tool_cartridge', 'orcalti'));
                }
            };
        ...
        updatePrivacySettings: function(toolInfo){
            if(!toolInfo || !toolInfo.toolid){
                toolInfo = {
                    sendname: M.mod_orcalti.ORCALTI_SETTING_DELEGATE,
                    sendemailaddr: M.mod_orcalti.ORCALTI_SETTING_DELEGATE,
                    acceptgrades: M.mod_orcalti.ORCALTI_SETTING_DELEGATE
                }
            }
            ...
            for(setting in privacyControls){
                if(privacyControls.hasOwnProperty(setting)){
                    ...
                    if(settingValue == M.mod_orcalti.ORCALTI_SETTING_NEVER){
                        control.set('disabled', true);
                        control.set('checked', false);
                        control.set('title', M.util.get_string('forced_help', 'orcalti'));
                    } else if(settingValue == M.mod_orcalti.ORCALTI_SETTING_ALWAYS){
                        control.set('disabled', true);
                        control.set('checked', true);
                        control.set('title', M.util.get_string('forced_help', 'orcalti'));
                    } else if(settingValue == M.mod_orcalti.ORCALTI_SETTING_DELEGATE){
        ...
        addOptGroups: function(){
            ...
            if(typeSelector.one('option[courseTool=1]')){
                ...
                var globalGroup = Y.Node.create('<optgroup />')
                                    ...
                                    .set('label', M.util.get_string('global_tool_types', 'orcalti'));

                var courseGroup = Y.Node.create('<optgroup />')
                                    .set('id', 'course_tool_group')
                                    .set('label', M.util.get_string('course_tool_types', 'orcalti'));
        ...
        createTypeEditorButtons: function(){
            ...
            editIcon.on('click', function(e){
                ...
                if(self.getSelectedToolTypeOption().getAttribute('editable')){
                    ...
                    alert(M.util.get_string('cannot_edit', 'orcalti'));
                }
            });
            ...
            deleteIcon.on('click', function(e){
                ...
                if(self.getSelectedToolTypeOption().getAttribute('editable')){
                    if(confirm(M.util.get_string('delete_confirmation', 'orcalti'))){
                        self.deleteTool(toolTypeId);
                    }
                } else {
                    alert(M.util.get_string('cannot_delete', 'orcalti'));
                }
            });
        ...
        addToolType: function(toolType){
            ...
            require(["core/notification"], function (notification) {
                notification.addNotification({
                    message: M.util.get_string('tooltypeadded', 'orcalti'),
                    type: "success"
                });
            });
        },
        ...
        updateToolType: function(toolType){
            ...
            require(["core/notification"], function (notification) {
                notification.addNotification({
                    message: M.util.get_string('tooltypeupdated', 'orcalti'),
                    type: "success"
                });
            });
        },
            ...
            Y.io(self.settings.instructor_tool_type_edit_url + '&action=delete&typeid=' + toolTypeId, {
                on: {
                    success: function(){
                        ...
                        require(["core/notification"], function (notification) {
                            notification.addNotification({
                                message: M.util.get_string('tooltypedeleted', 'orcalti'),
                                type: "success"
                            });
                        });
                    },
                    failure: function(){
                        require(["core/notification"], function (notification) {
                            notification.addNotification({
                                message: M.util.get_string('tooltypenotdeleted', 'orcalti'),
                                type: "problem"
                            });
                        });
                    }
