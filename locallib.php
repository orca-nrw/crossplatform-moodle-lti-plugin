...
define('ORCALTI_URL_DOMAIN_REGEX', '/(?:https?:\/\/)?(?:www\.)?([^\/]+)(?:\/|$)/i');

define('ORCALTI_LAUNCH_CONTAINER_DEFAULT', 1);
define('ORCALTI_LAUNCH_CONTAINER_EMBED', 2);
define('ORCALTI_LAUNCH_CONTAINER_EMBED_NO_BLOCKS', 3);
define('ORCALTI_LAUNCH_CONTAINER_WINDOW', 4);
define('ORCALTI_LAUNCH_CONTAINER_REPLACE_MOODLE_WINDOW', 5);

define('ORCALTI_TOOL_STATE_ANY', 0);
define('ORCALTI_TOOL_STATE_CONFIGURED', 1);
define('ORCALTI_TOOL_STATE_PENDING', 2);
define('ORCALTI_TOOL_STATE_REJECTED', 3);
define('ORCALTI_TOOL_PROXY_TAB', 4);

define('ORCALTI_TOOL_PROXY_STATE_CONFIGURED', 1);
define('ORCALTI_TOOL_PROXY_STATE_PENDING', 2);
define('ORCALTI_TOOL_PROXY_STATE_ACCEPTED', 3);
define('ORCALTI_TOOL_PROXY_STATE_REJECTED', 4);

define('ORCALTI_SETTING_NEVER', 0);
define('ORCALTI_SETTING_ALWAYS', 1);
define('ORCALTI_SETTING_DELEGATE', 2);

define('ORCALTI_COURSEVISIBLE_NO', 0);
define('ORCALTI_COURSEVISIBLE_PRECONFIGURED', 1);
define('ORCALTI_COURSEVISIBLE_ACTIVITYCHOOSER', 2);

define('ORCALTI_VERSION_1', 'LTI-1p0');
define('ORCALTI_VERSION_2', 'LTI-2p0');
define('ORCALTI_VERSION_1P3', '1.3.0');
define('ORCALTI_RSA_KEY', 'RSA_KEY');
define('ORCALTI_JWK_KEYSET', 'JWK_KEYSET');

define('ORCALTI_DEFAULT_ORGID_SITEID', 'SITEID');
define('ORCALTI_DEFAULT_ORGID_SITEHOST', 'SITEHOST');

define('ORCALTI_ACCESS_TOKEN_LIFE', 3600);

// Standard prefix for JWT claims.
define('ORCALTI_JWT_CLAIM_PREFIX', 'https://purl.imsglobal.org/spec/lti');
 ...
 * @return array
 */
function orcalti_get_jwt_message_type_mapping() {
    return array(
 ...
 * @return array
 */
function orcalti_get_jwt_claim_mapping() {
    return array(
        'accept_copy_advice' => [
            'suffix' => 'dl',
 ...
 * @since  Moodle 3.9
 */
function orcalti_get_instance_type(object $instance) : ?object {
    if (empty($instance->typeid)) {
        if (!$tool = orcalti_get_tool_by_url_match($instance->toolurl, $instance->course)) {
            $tool = orcalti_get_tool_by_url_match($instance->securetoolurl,  $instance->course);
        }
        return $tool;
    }
    return orcalti_get_type($instance->typeid);
}
...
function orcalti_get_launch_data($instance, $nonce = '') {
    ...
    if (isset($tool->toolproxyid)) {
        ...
        } else if ($ltiversion === ORCALTI_VERSION_1P3) {
            $key = $tool->clientid;
    ...
    if (orcalti_request_is_using_ssl() && !empty($instance->securetoolurl)) {
    ...
    $url = new \moodle_url('/mod/orcalti/return.php', $returnurlparams);
    $returnurl = $url->out(false);

    if (isset($typeconfig['forcessl']) && ($typeconfig['forcessl'] == '1')) {
        $returnurl = orcalti_ensure_url_is_https($returnurl);
    }

    $target = '';
    switch($launchcontainer) {
        case ORCALTI_LAUNCH_CONTAINER_EMBED:
        case ORCALTI_LAUNCH_CONTAINER_EMBED_NO_BLOCKS:
            $target = 'iframe';
            break;
        case ORCALTI_LAUNCH_CONTAINER_REPLACE_MOODLE_WINDOW:
            $target = 'frame';
            break;
        case ORCALTI_LAUNCH_CONTAINER_WINDOW:
            $target = 'window';
            break;
    }
    if (!empty($target)) {
    ...
    foreach (array_keys($plugins) as $plugin) {
        $pluginparams = component_callback('orcaltisource_'.$plugin, 'before_launch',
    ...
    if ((!empty($key) && !empty($secret)) || ($ltiversion === ORCALTI_VERSION_1P3)) {
        if ($ltiversion !== ORCALTI_VERSION_1P3) {
            $parms = orcalti_sign_parameters($requestparams, $endpoint, 'POST', $key, $secret);
        } else {
            $parms = orcalti_sign_jwt($requestparams, $endpoint, $key, $typeid, $nonce);
        }
 ...
 */
function orcalti_register($toolproxy) {
    $endpoint = $toolproxy->regurl;

    // Change the status to pending.
    $toolproxy->state = ORCALTI_TOOL_PROXY_STATE_PENDING;
    orcalti_update_tool_proxy($toolproxy);

    $requestparams = orcalti_build_registration_request($toolproxy);

    $content = orcalti_post_launch_html($requestparams, $endpoint, false);
 ...
 */
function orcalti_build_registration_request($toolproxy) {
    $key = $toolproxy->guid;
    $secret = $toolproxy->secret;
    ...
    // Add the profile URL.
    $profileservice = orcalti_get_service_by_name('profile');
    ...
    $url = new \moodle_url('/mod/orcalti/externalregistrationreturn.php', $returnurlparams);
    $returnurl = $url->out(false);
 ...
 * @return string
 */
function orcalti_get_organizationid($typeconfig) {
    global $CFG;
    // Default the organizationid if not specified.
    if (empty($typeconfig['organizationid'])) {
        if (($typeconfig['organizationid_default'] ?? ORCALTI_DEFAULT_ORGID_SITEHOST) == ORCALTI_DEFAULT_ORGID_SITEHOST) {
            $urlparts = parse_url($CFG->wwwroot);
 ...
 * @return stdClass
 */
function orcalti_build_sourcedid($instanceid, $userid, $servicesalt, $typeid = null, $launchid = null) {
    $data = new \stdClass();
...
function orcalti_build_request($instance, $typeconfig, $course, $typeid = null, $islti2 = false) {
    ...
    if (!empty($instance->cmid)) {
        $intro = format_module_intro('orcalti', $instance, $instance->cmid);
...
function orcalti_build_standard_request($instance, $orgid, $islti2, $messagetype = 'basic-lti-launch-request') {
    ...
    return orcalti_build_standard_message($instance, $orgid, $ltiversion, $messagetype);
}
 ...
 */
function orcalti_build_standard_message($instance, $orgid, $ltiversion, $messagetype = 'basic-lti-launch-request') {
    global $CFG;
    ...
    if (!empty($CFG->mod_orcalti_institution_name)) {
        $requestparams['tool_consumer_instance_name'] = trim(html_to_text($CFG->mod_orcalti_institution_name, 0));
    } else {
...
function orcalti_build_custom_parameters($toolproxy, $tool, $instance, $params, $customstr, $instructorcustomstr, $islti2) {
    ...
    if ($islti2) {
        ...
        $custom = array_merge($custom, orcalti_get_custom_parameters($toolproxy, $tool, $params, $settings));
        if (!empty($instance->course)) {
            $settings = orcalti_get_tool_settings($tool->toolproxyid, $instance->course);
            $custom = array_merge($custom, orcalti_get_custom_parameters($toolproxy, $tool, $params, $settings));
            if (!empty($instance->id)) {
                $settings = orcalti_get_tool_settings($tool->toolproxyid, $instance->course, $instance->id);
                $custom = array_merge($custom, orcalti_get_custom_parameters($toolproxy, $tool, $params, $settings));
            }
        }
    }
...
function orcalti_build_content_item_selection_request($id, $course, moodle_url $returnurl, 
    ...
    $orgid = orcalti_get_organizationid($typeconfig);
    $standardparams = orcalti_build_standard_message(null, $orgid, $tool->ltiversion, 'ContentItemSelectionRequest');
 ...
 */
function orcalti_verify_with_keyset($jwtparam, $keyseturl, $clientid) {
    // Attempts to retrieve cached keyset.
    $cache = cache::make('mod_orcalti', 'keyset');
 ...
 * @throws ExpiredException             Provided JWT has since expired, as defined by the 'exp' claim
 */
function orcalti_verify_jwt_signature($typeid, $consumerkey, $jwtparam) {
    $tool = orcalti_get_type($typeid);
    ...
    if (empty($typeconfig['keytype']) || $typeconfig['keytype'] === ORCALTI_RSA_KEY) {
        ...
        orcalti_verify_with_keyset($jwtparam, $keyseturl, $tool->clientid);
    } else {
 ...
 */
function orcalti_convert_content_items($param) {
    $items = array();
...
function orcalti_get_tool_table($tools, $id) {
    global $OUTPUT;
    $html = '';

    $typename = get_string('typename', 'orcalti');
    $baseurl = get_string('baseurl', 'orcalti');
    $action = get_string('action', 'orcalti');
    $createdon = get_string('createdon', 'orcalti');
    ...
    if (!empty($tools)) {
        ...
        foreach ($tools as $type) {
            ...
            $accept = get_string('accept', 'orcalti');
            $update = get_string('update', 'orcalti');
            $delete = get_string('delete', 'orcalti');

            if (empty($type->toolproxyid)) {
                $baseurl = new \moodle_url('/mod/orcalti/typessettings.php', array(
                        'action' => 'accept',
                        'id' => $type->id,
                        'sesskey' => sesskey(),
                        'tab' => $id
                ...
                $baseurl = new \moodle_url('/mod/orcalti/toolssettings.php', array(
                        'action' => 'accept',
                        'id' => $type->id,
                        'sesskey' => sesskey(),
                        'tab' => $id
            ...
            if ($type->state == ORCALTI_TOOL_STATE_CONFIGURED) {
                $accepthtml = '';
            }

            if ($type->state != ORCALTI_TOOL_STATE_REJECTED) {
                $deleteaction = 'reject';
                $delete = get_string('reject', 'orcalti');
            }
            ...
            if (($type->state != ORCALTI_TOOL_STATE_REJECTED) || empty($type->toolproxyid)) {
        ...
        $html .= get_string('no_' . $id, 'orcalti');
    }
 ...
 */
function orcalti_get_tool_proxy_table($toolproxies, $id) {
    global $OUTPUT;

    if (!empty($toolproxies)) {
        $typename = get_string('typename', 'orcalti');
        $url = get_string('registrationurl', 'orcalti');
        $action = get_string('action', 'orcalti');
        $createdon = get_string('createdon', 'orcalti');
...
EOD;
        foreach ($toolproxies as $toolproxy) {
            ...
            $accept = get_string('register', 'orcalti');
            $update = get_string('update', 'orcalti');
            $delete = get_string('delete', 'orcalti');

            $baseurl = new \moodle_url('/mod/orcalti/registersettings.php', array(
                    'action' => 'accept',
                    'id' => $toolproxy->id,
                    'sesskey' => sesskey(),
                    'tab' => $id
            ...
            $registerurl = new \moodle_url('/mod/orcalti/register.php', array(
                    'id' => $toolproxy->id,
                    'sesskey' => sesskey(),
                    'tab' => 'tool_proxy'
                ));
 ...
 */
function orcalti_get_enabled_capabilities($tool) {
    if (!isset($tool)) {
        return array();
    }
 ...
 */
function orcalti_get_custom_parameters($toolproxy, $tool, $params, $parameters) {
    $retval = array();
    foreach ($parameters as $key => $val) {
        $key2 = orcalti_map_keyname($key);
        $val = orcalti_parse_custom_parameter($toolproxy, $tool, $params, $val, true);
        $retval['custom_'.$key2] = $val;
        if ($key != $key2) {
            $retval['custom_'.$key] = $val;
        }
            ...
            if (!$islti2 || in_array($value1, $enabledcapabilities)) {
                ...
                if (array_key_exists($value1, $capabilities)) {
                    ...
                    if ($val) {
                        ...
                        $value = orcalti_calculate_custom_parameter($value1);
                    }
                } else {
                    $val = $value;
                    $services = orcalti_get_services();
 ...
 */
function orcalti_calculate_custom_parameter($value) {
    global $USER, $COURSE;
 ...
 */
function orcalti_map_keyname($key, $tolower = true) {
    if ($tolower) {
        $newkey = '';
...
function orcalti_get_ims_role($user, $cmid, $courseid, $islti2) {
    ...
    if (empty($cmid)) {
        ...
        if (has_capability('mod/orcalti:manage', $context)) {
            array_push($roles, 'Instructor');
        } else {
            array_push($roles, 'Learner');
        }
 ...
 */
function orcalti_get_type_config($typeid) {
    global $DB;
...
function orcalti_get_tools_by_url($url, $state, $courseid = null) {
    $domain = orcalti_get_domain_from_url($url);

    return orcalti_get_tools_by_domain($domain, $state, $courseid);
}

function orcalti_get_tools_by_domain($domain, $state = null, $courseid = null) {
    global $DB, $SITE;
 ...
 * @return array
 */
function orcalti_filter_get_types($course) {
    global $DB;
 ...
 *
 * @param array $tools An array of orcalti_types records
 * @param int $state One of the ORCALTI_TOOL_STATE_* constants
 * @return array
 */
function orcalti_filter_tool_types(array $tools, $state) {
    $return = array();
...
function orcalti_get_orcalti_types_by_course($courseid, $coursevisible = null) {
    ...
    if ($coursevisible === null) {
        $coursevisible = [ORCALTI_COURSEVISIBLE_PRECONFIGURED, ORCALTI_COURSEVISIBLE_ACTIVITYCHOOSER];
    }

    list($coursevisiblesql, $coursevisparams) = $DB->get_in_or_equal($coursevisible, SQL_PARAMS_NAMED, 'coursevisible');
    $courseconds = [];
    if (has_capability('mod/orcalti:addmanualinstance', context_course::instance($courseid))) {
        $courseconds[] = "course = :courseid";
    }
    if (has_capability('mod/orcalti:addpreconfiguredinstance', context_course::instance($courseid))) {
...
function orcalti_get_types_for_add_instance() {
    global $COURSE;
    $admintypes = orcalti_get_orcalti_types_by_course($COURSE->id);

    $types = array();
    if (has_capability('mod/orcalti:addmanualinstance', context_course::instance($COURSE->id))) {
        $types[0] = (object)array('name' => get_string('automatic', 'orcalti'), 'course' => 0, 'toolproxyid' => null);
    }
...
function orcalti_get_domain_from_url($url) {
    $matches = array();

    if (preg_match(ORCALTI_URL_DOMAIN_REGEX, $url, $matches)) {
        return $matches[1];
    }
}

function orcalti_get_tool_by_url_match($url, $courseid = null, $state = ORCALTI_TOOL_STATE_CONFIGURED) {
    $possibletools = orcalti_get_tools_by_url($url, $state, $courseid);

    return orcalti_get_best_tool_by_url($url, $possibletools, $courseid);
}

function orcalti_get_url_thumbprint($url) {
...
function orcalti_get_best_tool_by_url($url, $tools, $courseid = null) {
    if (count($tools) === 0) {
        return null;
    }

    $urllower = orcalti_get_url_thumbprint($url);

    foreach ($tools as $tool) {
        $tool->_matchscore = 0;

        $toolbaseurllower = orcalti_get_url_thumbprint($tool->baseurl);
...
function orcalti_get_shared_secrets_by_key($key) {
    global $DB;
 ...
 *
 */
function orcalti_get_type_config_from_instance($id) {
    global $DB;

    $instance = $DB->get_record('orcalti', array('id' => $id));
...
function orcalti_get_type_type_config($id) {
    ...
    if (isset($config['organizationid_default'])) {
        ...
        $type->lti_organizationid_default = ORCALTI_DEFAULT_ORGID_SITEHOST;
    }
...
function orcalti_update_type($type, $config) {
    global $DB, $CFG;

    orcalti_prepare_type_for_save($type, $config);

    if (orcalti_request_is_using_ssl() && !empty($type->secureicon)) {
...
function orcalti_add_type($type, $config) {
    global $USER, $SITE, $DB;
    ...
    if (!isset($type->ltiversion)) {
        $type->ltiversion = ORCALTI_VERSION_1;
    }
 ...
 *
 * @param array $toolproxies An array of orcalti_tool_proxies records
 * @param int $state One of the ORCALTI_TOOL_PROXY_STATE_* constants
 *
 * @return array
 */
function orcalti_filter_tool_proxy_types(array $toolproxies, $state) {
    $return = array();
 ...
 * @return object
 */
function orcalti_get_tool_proxy_from_guid($toolproxyguid) {
    global $DB;

    $toolproxy = $DB->get_record('orcalti_tool_proxies', array('guid' => $toolproxyguid));
 ...
 */
function orcalti_get_tool_proxies_from_registration_url($regurl) {
    global $DB;

    return $DB->get_records_sql(
        'SELECT * FROM {orcalti_tool_proxies}
 ...
 */
function orcalti_get_tool_proxy($id) {
    global $DB;

    $toolproxy = $DB->get_record('orcalti_tool_proxies', array('id' => $id));
    return $toolproxy;
}
...
function orcalti_get_tool_proxies($orphanedonly) {
    ...
    if ($orphanedonly) {
        ...
        return $DB->get_records('orcalti_tool_proxies', null, 'state DESC, timemodified DESC');
    }
}
 ...
 * @return mixed  Tool Proxy details
 */
function orcalti_get_tool_proxy_config($id) {
 ...
 */
function orcalti_add_tool_proxy($config) {
    global $USER, $DB;
    ...
    if (isset($config->lti_capabilities)) {
        ...
        $toolproxy->capabilityoffered = implode("\n", array_keys(orcalti_get_capabilities()));
    }
    if (isset($config->lti_services)) {
        ...
        $servicenames = array_map($func, orcalti_get_services());
        $toolproxy->serviceoffered = implode("\n", $servicenames);
    }
    if (isset($config->toolproxyid) && !empty($config->toolproxyid)) {
        $toolproxy->id = $config->toolproxyid;
        if (!isset($toolproxy->state) || ($toolproxy->state != ORCALTI_TOOL_PROXY_STATE_ACCEPTED)) {
            $toolproxy->state = ORCALTI_TOOL_PROXY_STATE_CONFIGURED;
            $toolproxy->guid = random_string();
            $toolproxy->secret = random_string();
        }
        $id = orcalti_update_tool_proxy($toolproxy);
    } else {
        $toolproxy->state = ORCALTI_TOOL_PROXY_STATE_CONFIGURED;
        ...
        $toolproxy->guid = random_string();
        $toolproxy->secret = random_string();
        $id = $DB->insert_record('orcalti_tool_proxies', $toolproxy);
    }
 ...
 * @return int    Record id number
 */
function orcalti_update_tool_proxy($toolproxy) {
    global $DB;

    $toolproxy->timemodified = time();
    $id = $DB->update_record('orcalti_tool_proxies', $toolproxy);
 ...
 */
function orcalti_get_tool_settings($toolproxyid, $courseid = null, $instanceid = null) {
    global $DB;

    $settings = array();
    if ($toolproxyid > 0) {
        $settingsstr = $DB->get_field('orcalti_tool_settings', 'settings', array('toolproxyid' => $toolproxyid,
            'course' => $courseid, 'coursemoduleid' => $instanceid));
    } else {
        $settingsstr = $DB->get_field('orcalti_tool_settings', 'settings', array('typeid' => -$toolproxyid,
            'course' => $courseid, 'coursemoduleid' => $instanceid));
    }
 ...
 * @param int    $instanceid    Id of course module (null if system or context settings)
 */
function orcalti_set_tool_settings($settings, $toolproxyid, $courseid = null, $instanceid = null) {
    global $DB;

    $json = json_encode($settings);
    if ($toolproxyid >= 0) {
        $record = $DB->get_record('orcalti_tool_settings', array('toolproxyid' => $toolproxyid,
            'course' => $courseid, 'coursemoduleid' => $instanceid));
    } else {
        $record = $DB->get_record('orcalti_tool_settings', array('typeid' => -$toolproxyid,
            'course' => $courseid, 'coursemoduleid' => $instanceid));
    }
    if ($record !== false) {
        ...
        $DB->insert_record('orcalti_tool_settings', $record);
    }
}
 ...
 * @return array|null
 */
function orcalti_sign_parameters($oldparms, $endpoint, $method, $oauthconsumerkey, $oauthconsumersecret) {
 ...
 * @return array|null
 */
function orcalti_sign_jwt($parms, $endpoint, $oauthconsumerkey, $typeid = 0, $nonce = '') {
    global $CFG;

    if (empty($typeid)) {
        $typeid = 0;
    }
    $messagetypemapping = orcalti_get_jwt_message_type_mapping();
    ...
    $claimmapping = orcalti_get_jwt_claim_mapping();
    $payload = array(
        'nonce' => $nonce,
        'iat' => $now,
        'exp' => $now + 60,
    ...
    $payload[ORCALTI_JWT_CLAIM_PREFIX . '/claim/deployment_id'] = strval($typeid);
    $payload[ORCALTI_JWT_CLAIM_PREFIX . '/claim/target_link_uri'] = $endpoint;

    foreach ($parms as $key => $value) {
        $claim = ORCALTI_JWT_CLAIM_PREFIX;
 ...
 * @throws moodle_exception
 */
function orcalti_convert_from_jwt($typeid, $jwtparam) {
    ...
    if ($ok) {
        orcalti_verify_jwt_signature($typeid, $claims['iss'], $jwtparam);
        $params['oauth_consumer_key'] = $claims['iss'];
        foreach (orcalti_get_jwt_claim_mapping() as $key => $mapping) {
            $claim = ORCALTI_JWT_CLAIM_PREFIX;
            if (!empty($mapping['suffix'])) {
                $claim .= "-{$mapping['suffix']}";
            }
            ...
            $claim = ORCALTI_JWT_CLAIM_PREFIX . '/claim/ext';
            if (isset($claims[$claim])) {
                $ext = $claims[$claim];
                if (is_array($ext)) {
    ...
    if (isset($params['content_items'])) {
        $params['content_items'] = orcalti_convert_content_items($params['content_items']);
    }
    $messagetypemapping = orcalti_get_jwt_message_type_mapping();
...
function orcalti_get_launch_container($lti, $toolconfig) {
    ...
    if (empty($launchcontainer) || $launchcontainer == ORCALTI_LAUNCH_CONTAINER_DEFAULT) {
        $launchcontainer = ORCALTI_LAUNCH_CONTAINER_EMBED_NO_BLOCKS;
    }
    ...
    if ($devicetype === core_useragent::DEVICETYPE_MOBILE || $devicetype === core_useragent::DEVICETYPE_TABLET ) {
        $launchcontainer = ORCALTI_LAUNCH_CONTAINER_REPLACE_MOODLE_WINDOW;
    }

    return $launchcontainer;
}

function orcalti_request_is_using_ssl() {
    global $CFG;
    return (stripos($CFG->wwwroot, 'https://') === 0);
}

function orcalti_ensure_url_is_https($url) {
    if (!strstr($url, '://')) {
        $url = 'https://' . $url;
    } else {
 ...
 * @return bool
 */
function orcalti_should_log_request($rawbody) {
    global $CFG;

    if (empty($CFG->mod_orcalti_log_users)) {
        return false;
    }

    $logusers = explode(',', $CFG->mod_orcalti_log_users);
    if (empty($logusers)) {
        return false;
    }
 ...
 * @param string $rawbody
 */
function orcalti_log_request($rawbody) {
    if ($tempdir = make_temp_directory('mod_orcalti', false)) {
        if ($tempfile = tempnam($tempdir, 'mod_orcalti_request'.date('YmdHis'))) {
            $content  = "Request Headers:\n";
            foreach (moodle\mod\orcalti\OAuthUtil::get_headers() as $header => $value) {
 ...
 */
function orcalti_log_response($responsexml, $e = null) {
    if ($tempdir = make_temp_directory('mod_orcalti', false)) {
        if ($tempfile = tempnam($tempdir, 'mod_orcalti_response'.date('YmdHis'))) {
            $content = '';
 ...
 */
function orcalti_get_type_config_by_instance($instance) {
    $typeid = null;
    if (empty($instance->typeid)) {
        $tool = orcalti_get_tool_by_url_match($instance->toolurl, $instance->course);
        if ($tool) {
            $typeid = $tool->id;
        }
    } else {
    ...
    if (!empty($typeid)) {
        return orcalti_get_type_config($typeid);
    }
    return array();
}
 ...
 */
function orcalti_force_type_config_settings($instance, array $typeconfig) {
    $forced = array(
    ...
    foreach ($forced as $instanceparam => $typeconfigparam) {
        if (array_key_exists($typeconfigparam, $typeconfig) && $typeconfig[$typeconfigparam] != ORCALTI_SETTING_DELEGATE) {
 ...
 */
function orcalti_get_capabilities() {
 ...
 *
 * @return bool|\mod_orcalti\local\orcaltiservice\service_base Service
 */
 ...
 *
 * @param \mod_orcalti\local\orcaltiservice\service_base[] $services Array of services
 * @param string $resourceid  ID of resource
 *
 * @return mod_orcalti\local\orcaltiservice\service_base Service
 */
function orcalti_get_service_by_resource_id($services, $resourceid) {
 ...
 */
function orcalti_get_permitted_service_scopes($type, $typeconfig) {
 ...
 */
function orcalti_get_contexts($json) {
 ...
 */
function orcalti_get_fqid($contexts, $id) {
 ...
 * @since Moodle 3.1
 */
function orcalti_is_cartridge($url) {
 ...
 * @since Moodle 3.1
 */
function orcalti_load_cartridge($url, $map, $propertiesmap = array()) {
    global $CFG;
 ...
 */
function orcalti_new_access_token($typeid, $scopes) {
    global $DB;
    ...
    $newtoken->validuntil = $newtoken->timecreated + ORCALTI_ACCESS_TOKEN_LIFE;
