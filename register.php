...
foreach ($toolproxies as $key => $toolproxy) {
    if (($toolproxy->state == ORCALTI_TOOL_PROXY_STATE_PENDING) || ($toolproxy->state == ORCALTI_TOOL_PROXY_STATE_ACCEPTED)) {
...
$redirect = new moodle_url('/mod/orcalti/toolproxies.php');
if ($duplicate) {
    redirect($redirect,  get_string('duplicateregurl', 'orcalti'));
}


$profileservice = orcalti_get_service_by_name('profile');
if (empty($profileservice)) {
    redirect($redirect,  get_string('noprofileservice', 'orcalti'));
}
...
$PAGE->set_title("{$SITE->shortname}: " . get_string('toolproxyregistration', 'orcalti'));

// Print the page header.
echo $OUTPUT->header();

echo $OUTPUT->heading(get_string('toolproxyregistration', 'orcalti'));

echo $OUTPUT->box_start('generalbox');

// Request the registration request content with an object tag.
$registration = new moodle_url('/mod/orcalti/registration.php',
...
echo get_string('register_warning', 'orcalti');
echo "\n</p>\n";
...
$resize = '
        ...
        //<![CDATA[
            ...
            function doOnload() {
                window.clearTimeout(mod_orcalti_timer);
            }
            var mod_orcalti_timer = window.setTimeout(doReveal, 20000);
